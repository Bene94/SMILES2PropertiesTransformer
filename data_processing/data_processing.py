...
@click.option('--file_path', '-p',default=["deepchem/tox21/test"], help='Location of raw data', multiple=True)
@click.option('--save_path', '-s', default="deepchem/tox21", help='Location of output data')
...
@click.option('--source', default='noSplit', help='see if to select COSMO or EXP')
@click.option('--pre', default='train', help='sets the prefix for the output files')
...
def main(file_path, save_path, vocab_path, ul, ll, frac, aug, max_aug,seed, ow, h2o, source, pre):
    processing(file_path, save_path, vocab_path, ul, ll, frac, aug, max_aug, seed, ow, h2o, source, pre)

def processing(folder_name, save_path, vocab_path, ul, ll, frac, aug, max_aug, seed, ow, h2o, source, pre):
...
def get_paths(save_path, vocab_path):
    if os.environ.get('XPRUN_NAME') is not None:
        ...
        alias_path = '../raw_data/alias/alias_dict_tox21.npy'
