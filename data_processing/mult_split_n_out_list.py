import data_processing as dc
import numpy as np
import os
from numpy.random import default_rng
## function to create multiple splits of the data

file_path = ["brouwer_exp_c"]
vocab_path = "vocab"
ow = True

exclude_H2O = True
only_H2O = False 


data_path = '../data/'
#data_path = '/mnt/xprun/data/'


num_splits = [10, 50, 100, 200]

comp_list, systems, __ = dc.get_comp_list(file_path, vocab_path)
n_unique = len(systems)
index_list = np.arange(0,n_unique)
# find systems with water as solvent

# number of sets 
num_sets = 200

for num_split in num_splits:
    if exclude_H2O:
        h2o_index = systems[(systems.solvent == 'O') | (systems.solute == 'O')].index
        index_list = np.setdiff1d(index_list, h2o_index)
        save_path = "data_exp_noH2O_" + str(num_split)  + '_V2'
    elif only_H2O:
        index_list = systems[systems.solvent == 'O'].index
        index_list = np.array(index_list)
        save_path = "data_exp_onlyH2O"
        num_split = 20

    # make the index list longer
    len_index_list = num_sets * int(n_unique / num_split)
    index_list_full = []
    # append the index list to itself until it is long enough
    rng = default_rng(0)
    while len(index_list_full) < len_index_list:
        rng.shuffle(index_list)
        index_list_full = np.append(index_list_full, index_list)

    index_list_full = index_list[0:len_index_list]
    index_list_full = index_list.astype(int)
     
    # shuffle the index list
    rng = default_rng(0)
    rng.shuffle(index_list)
    # break the index list into chunks of size num_splits
    index_list_chunks = np.array_split(index_list_full, num_sets)
    index_list_chunks = [np.unique(x) for x in index_list_chunks]

    for i, index in enumerate(index_list_chunks):
        print("\n")
        print(str(i) + "th split \n")
        save_path_temp = save_path + "/" +str(i)
        # check if the directory exists
        if not os.path.exists(data_path+save_path_temp):
            os.makedirs(data_path+save_path_temp)
        dc.processing_n_out(file_path, save_path_temp, vocab_path, ow, comp_list, systems, index)